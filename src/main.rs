...
fn handle_keys(tcod: &mut Tcod, game: &mut Game, objects: &mut Vec<Object>) -> PlayerAction {
    ...
    match (tcod.key, tcod.key.text(), player_alive) {
        ...
        (Key { code: Text, .. }, "<", true) => {
            // go down stairs, if the player is on item
...
fn render_all(tcod: &mut Tcod, game: &mut Game, objects: &[Object], fov_recompute: bool) {
    ...
    if let Some(fighter) = objects[PLAYER].fighter {
        tcod.root.print_ex(
            ...
            BackgroundFlag::None,
            TextAlignment::Left,
            format!("HP: {}/{}", fighter.hp, objects[PLAYER].max_hp(game)),
        );
    }
    ...
    // show the player's stats
    let hp = objects[PLAYER].fighter.map_or(9, |f| f.hp);
...
fn move_by(id: usize, dx: i32, dy: i32, map: &Map, objects: &mut [Object]) {
    ...
    let next_x = x + dx;
    let next_y = y + dy;
    if !is_blocked(next_x, next_y, map, objects) {
...
fn move_towards(id: usize, target_x: i32, target_y: i32, map: &Map, objects: &mut [Object]) {
    ...
    // normalize it to length 1, then round it and convert to integer so the movement is restricted to the map grid
...
fn player_death(player: &mut Object, game: &mut Game) {
    ...
    // for addend effect, transform the player into a corpse
    player.char = '%';
    player.color = DARK_RED;
}

fn monster_death(monster: &mut Object, game: &mut Game) {
    // transform it into a nasty copse! it doesn't block, can't be attacked and doesn't move
    game.messages.add(
        format!(
...
fn get_names_under_mouse(mouse: Mouse, objects: &[Object], fov_map: &FovMap) -> String {
    ...
    names.join(", ") // join the names, separated by comes
}
...
fn menu<T: AsRef<str>>(header: &str, options: &[T], width: i32, root: &mut Root) -> Option<usize> {
    ...
    // present the root console to player and wait for a key-press
    root.flush();
...
fn cast_lightning(
    ...
    if let Some(monster_id) = monster_id {
        ...
        game.messages.add(
            format!(
                "A lightning bolt strikes the {} with a loud monster! The damage is {} hit points",
...
fn cast_fireball(
    ...
    game.messages.add(
        "Left-click a target file for the fireball, or right-click to cancel",
        LIGHT_CYAN,
    );
...
fn closest_monster(tcod: &Tcod, objects: &[Object], max_range: i32) -> Option<usize> {
    ...
    let mut closest_dist = (max_range + 1) as f32; // start with (slightly more than) maximum
...
fn target_tile(
    ...
    loop {
        ...
        // accept the target if the player clicked in FOV, and in case a range is specified, if it's in the target
...
fn toggle_equipment(
    ...
    if equipment.equipped {
        ...
        // if the slot is already used, dequip whatever is there first
